...
-- exhaustiveSel = filter where
-- I'll need a tree here, comming up!

-- endFirstSel: Selector which prefers jobs ending first (optimal)
endFirstSel :: (Ord a) => [Job a] -> [Job a]
endFirstSel = filter.sortBy endSort where
  filter (current:jobs') = current:(filter $ baseFilter (Just current) jobs')
  filter [] = []
